# cpp-learning

Learning C++ through projects. Building fundamentals toward physics simulations and scientific computing.

## Current Progress
- **Chapter 7:** Scope, Duration, and Linkage
- **CPE Certification:** In progress (target: August 23, 2026)

## Projects
| Project | Description | Concepts |
|---|---|---|
| Grade Classifier | Numeric grade to letter grade | if/else, bool functions |
| Number Classifier | Even/odd, positive/negative, digit count | bool functions, if/else chains, operators |
| Name Analyzer | Character count, initials extraction | std::string, .find(), .length() |
| Password Validator | Password strength checker (prototype) | std::string, bool checks, validation logic |

## Documentation
- **STYLE_GUIDE.md** — Naming conventions and formatting rules
- **reference.md** — C++ syntax reference and design guide

## Conventions
- **Files:** snake_case
- **Functions/Variables:** camelCase
